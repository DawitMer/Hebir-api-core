...
export class CampaignDto {
  ...
  @IsOptional() @IsUrl({ require_tld: false }) assetUrl?: string;
  @IsOptional()
  @IsUrl({ protocols: ['https'], require_tld: false })
  ctaUrl?: string;
  @IsOptional() @IsString() ctaLabel?: string;
  @IsArray() ageBands: string[];
  @IsArray() workCategories: string[];
  @IsEnum(CampaignState) state: CampaignState;
  @IsString() startsAt: string;
  @IsString() endsAt: string;
  @IsInt() @Min(1) @Max(60) requiredViewSeconds: number;
  @IsInt() @Min(300) rewardMinor: number;
  @IsInt() @Min(0) budgetMinor: number;
  @IsInt() @Min(1) deliveryWeight: number;
}
