...
export class AuthService {
  private readonly authContextCache = new Map<
    string,
    { value: { roles: UserRole[]; standing: AccountStanding } | null; expiresAt: number }
  >();
  ...
  async register(dto: RegisterDto) {
    ...
    const existing = await this.users.findOne({ where: { phoneNumber: dto.phoneNumber } });
    if (existing) {
  ...
  async refresh(rawRefreshToken: string) {
    ...
    return this.refreshTokens.manager.transaction(async (em) => {
      ...
      const user = await em.findOne(UserAccount, { where: { id: existing.userId } });
      if (!user) {
        throw new UnauthorizedException();
      }
  ...
  private async denyAccessJti(jti: string) {
    ...
    await this.redis.set(`${ACCESS_DENY_PREFIX}${jti}`, '1', 'EX', Math.max(ttlSec, 60));
  }
